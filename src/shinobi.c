...
void do_jutsus(CHAR_DATA *ch, char *argument)
{
	...
	if(move[0] == '\0' && ch->fighting == NULL){
		...
		stc(" #0|       #PTri - When 3 or more people with holy, shadow, fire, water. earth. air, sheres are in the room...              #0|\n\r",ch);
		stc("#0/|\\/|\\/|\\/|\\/|\\/|\\/|\\/|\\/|\\/|\\/|\\/|\\/|\\/|\\/|\\/|\\/|\\/|\\/|\\/|\\/|\\/|\\/|\\/|\\\n\r",ch);
		return;
	}
	...
	if(target[0] == '\0'){
		...
		if( (tarchar = get_char_room(ch,target)) == NULL){
			notarg = TRUE;
		}
...
void jutsu_vensai(CHAR_DATA *ch, CHAR_DATA *victim)
{
  ...
  if( !IS_NPC( victim ) && !IS_IMMUNE( victim, IMM_SUMMON ) && !ragnarok) {
...
void jutsu_butomi(CHAR_DATA *ch)
{
  ...
  if(ch->pcdata->powers[S_CLONETIMER] > 0 || ch->pcdata->followers> 0){
	...
	for(x = 0;x < r; x++){
		ch->pcdata->followers++;
		if(ch->pcdata->powers[S_HANDSHADOW] < 6) victim = create_mobile( get_mob_index( MOB_VNUM_MOUNT ) );
		...
		if(ch->fighting != NULL) set_fighting(victim,ch->fighting);
	}
...
void jutsu_shoju(CHAR_DATA *ch, char *argument)
{
	...
	int oldlev = ch->level;
        ...
        if (IS_SET(obj->quest,QUEST_SHINOBI))
          {
	...
	if(ch->practice < 2000){
		...
		obj->value[0] = 18000;
		ch->level = oldlev;
	         SET_BIT(obj->quest, QUEST_SHINOBI);
		 ch->practice -= 2000;
                 WAIT_STATE(ch, 20);
                 return;
          }
...
{
	int amount;
	if(victim->pcdata->powers[S_ELEMENT] == S_SHADOW){
...
void jutsu_beoutu(CHAR_DATA *ch)
{
	...
	if (IS_SPHERE(rch, SPHERE_HOLY)) count ++;}

	for(rch = ch->in_room->people; rch != NULL; rch = rch->next_in_room){
		if (IS_SPHERE(rch, SPHERE_HOLY)){
...
void jutsu_soothe(CHAR_DATA *ch, CHAR_DATA *victim)
{
	...
	int amount;
	if(!IS_SPHERE(victim, SPHERE_PYROMANCER) || (IS_SPHERE(victim,SPHERE_PYROMANCER) && victim->pcdata->powers[S_ELEMENT] == S_FIRE)){
		stc("You can not focus your energies on them.\n\r",ch);
		return;
	}
...
void jutsu_wash(CHAR_DATA *ch, CHAR_DATA *victim)
{
	...
	if(!IS_SET(victim->newbits,NEW_WASH)){
		...
		SET_BIT(victim->newbits,NEW_WASH);
	}
	else{
		...
		REMOVE_BIT(victim->newbits,NEW_WASH);
	}
	WAIT_STATE(ch,12);
...
void jutsu_viunemto(CHAR_DATA *ch, CHAR_DATA *victim)
{
	...
	if(!IS_SET(ch->newbits,NEW_VIUNEMTO)) SET_BIT(ch->newbits,NEW_VIUNEMTO);
