...
void do_reimb(CHAR_DATA * ch, char *argument)
{
  ...
  if (arg[0] == '\0')
  {
    send_to_char("Reimburse <target> <xp|qp|gold> <amount>\n\r", ch);
    return;
  }
...
void do_pstat(CHAR_DATA * ch, char *argument)
{
  ...
  sprintf(buf, "Level : %d. Trust : %d. Exp : %2.0f.", victim->level, victim->trust, victim->exp);
  send_to_char(buf, ch);
  ...
  send_to_char(buf, ch);
  sprintf(buf, "Gold : %d Silver : %d Copper : %d\n\r",victim->pcgold, victim->pcsilver, victim->pccopper);
  stc(buf, ch);
