...
bool is_note_to(CHAR_DATA * ch, NOTE_DATA * note)
{
  ...
  if ((get_trust(ch) == MAX_LEVEL) && (is_full_name("imp", note->to_list) ||
      is_full_name("imps", note->to_list) || is_full_name("implementor", note->to_list) ||
...
static void do_nwrite(CHAR_DATA * ch, char *argument)
{
  ...
  if (!ch->pcdata->in_progress->text) /* Are we continuing an old note or not? */
  {
    switch (ch->pcdata->board->force_type)
    {
      ...
      case DEF_EXCLUDE:
        sprintf(buf, "The recipient of this note must NOT include: \"" BOLD "%s" NO_COLOR "\".", ch->pcdata->board->names);
...
void do_board(CHAR_DATA * ch, char *argument)
{
    ...
    if (!argument[0])            /* show boards */
    {
        ...
        ("#R   -#D===============================================================#R-#n\n\r",
         ch);
        send_to_char
        ("   #D= #7NUM     NAME         NEW/ALL  READ/WRITE DESCRIPTION                    #D=\n\r",
         ch);
        send_to_char
        ("#R   -#D===============================================================#R-#n\n\r",
         ch);
        ...
        for (i = 0; i < MAX_BOARD; i++)
        {
            ...
            sprintf (buf,
                     "   #D= #7%s%2d    #7%s%-11s #R[#y%3d/#G%3d#R]#7 #R[ #y%s#R ]#7  #D%-29s#n #D=\n\r",
                     (!strcmp
                      (ch->pcdata->board->short_name,
                       boards[i].short_name)) ? "#7^1#7" : "#7", count,
                     (!strcmp
                      (ch->pcdata->board->short_name,
                       boards[i].short_name)) ? "#R->^1#7" : "  #7",
...
(get_trust(ch) < boards[i].write_level)
        ...
        ("#R   -#D===============================================================#R-#n\n\r",
         ch);
      sprintf(buf, "   You current board is #9%s#n.\n\r", ch->pcdata->board->short_name);
      send_to_char(buf, ch);
    ...
    sprintf (buf, "Current board changed to '#7%s#n'.\n\r#w%s.\n\r",
             boards[i].short_name,
             (get_trust (ch) < boards[i].write_level)
...
void handle_con_note_finish(DESCRIPTOR_DATA * d, char *argument)
{
  ...
  switch (tolower(argument[0]))
  {
    ...
    case 'p':                  /* post note */
      ...
      {
        sprintf(buf, "A new note has been posted by %s on board %d", ch->name, board_number(ch->pcdata->board) + 1);
        do_info(ch, buf);
      }
