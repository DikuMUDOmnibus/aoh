...
const struct olc_help_type help_table[] = {
  ...
  { "bits",      area_bits,        "Area global flags."		},
...
void show_quest_cmds(CHAR_DATA * ch)
{
  ...
  {
    sprintf(buf2, "%-26s", quest_table[quest].quest_name);
    strcat(buf1, buf2);
    if (++col % 3 == 0)
      strcat(buf1, "\n\r");
  }
...
void show_shop_cmds(CHAR_DATA * ch)
{
  ...
  {
    sprintf(buf2, "%-26s", shop_table[shop].shop_name);
    strcat(buf1, buf2);
    if (++col % 3 == 0)
      strcat(buf1, "\n\r");
  }
...
bool redit_rlist(CHAR_DATA * ch, char *argument)
{
  ...
  for (vnum = pArea->lvnum; vnum <= pArea->uvnum; vnum++)
  {
    ...
    {
      found = TRUE;
      sprintf(buf, "[%6d] %-40s [%-10s]", vnum, pRoom->name, flag_string(sector_flags, pRoom->sector_type));
      strcat(buf1, buf);
...
bool check_range(int lower, int upper)
{
  ...
  for (pArea = area_first; pArea; pArea = pArea->next)
  {
     ...
     * lower < area < upper
     */
    if ((lower <= pArea->lvnum && upper >= pArea->lvnum) || (upper >= pArea->uvnum && lower <= pArea->uvnum))
      cnt++;
...
bool redit_show(CHAR_DATA * ch, char *argument)
{
      ...
      else if (ed->type == ED_TYPE_PULL || ed->type == ED_TYPE_PRESS || ed->type == ED_TYPE_PUSH)
      {
        ...
        switch (ed->action)
        {
          case ED_ACTION_TELEPORT:
            sprintf(buf, " (Teleport to %d)]\n\r  [", ed->vnum);
            break;
          case ED_ACTION_OBJECT:
            sprintf(buf, " (Load object %d)]\n\r  [", ed->vnum);
            break;
          case ED_ACTION_ELEVATOR:
            sprintf(buf, " (Elevate to %d)]\n\r  [", ed->vnum);
            break;
          case ED_ACTION_SPELL:
            sprintf(buf, " (Cast spell %s)]\n\r  [", skill_table[ed->vnum].name);
            break;
        }
        strcat(buf1, buf);
  ...
  if (pRoom->roomtext)
  {
    ...
    for (rt = pRoom->roomtext; rt; rt = rt->next)
    {
      ...
      else if (rt->type == RT_SPELL)
      {
        ...
        strcat(buf1, rt->input);
        strcat(buf1, ") (");
        sprintf(buf2, "%s", skill_table[rt->power].name);
        strcat(buf1, buf2);
        strcat(buf1, ")]\n\r");
      }
      ...
      {
        sprintf(buf2, "[RT (keyword: %s) (type: %d) (name: %s)]\n\r",
          rt->input, rt->type, rt->name);
        strcat(buf1, buf2);
        sprintf(buf2, "[(power: %d) (mob: %d)]\n\r",
          rt->power, rt->mob);         
        strcat(buf1, buf2);
        sprintf(buf2, "[Out  : %s]\n\r[ChOut: %s]\n\r",
  ...
  for (door = 0; door < MAX_DIR; door++)
  {
    ...
    if ((pexit = pRoom->exit[door]))
    {
      ...
      sprintf(buf, "-%-5s to [%5d] Key: [%5d]", capitalize(dir_name[door]), pexit->to_room ? pexit->to_room->vnum : 0, pexit->key);
      strcat(buf1, buf);
...
void show_obj_values(CHAR_DATA * ch, OBJ_INDEX_DATA * obj)
{
  ...
  switch (obj->item_type)
  {
    ...
    case ITEM_GRENADE:
      sprintf(buf, "[v2] Explosive Power :   %d\n\r", obj->value[2]);
      send_to_char(buf, ch);
      break;
    ...
    case ITEM_PILL:
      sprintf(buf,
              ...
              : "none", obj->value[2] != -1 ? skill_table[obj->value[2]].name : "none", obj->value[3] != -1 ? skill_table[obj->value[3]].name : "none");
      send_to_char(buf, ch);
      break;
    ...
    case ITEM_WEAPON:
      sprintf(buf,
              "[v0] ItemAffects:    %d\n\r" "[v1] Dice (amount): [%d]\n\r" "[v2] Dice (size):   [%d]\n\r" "[v3] Type:           %s\n\r",
                obj->value[0], obj->value[1], obj->value[2], flag_string(weapon_flags, obj->value[3]));
      send_to_char(buf, ch);
...
flag_string(liquid_flags, obj->value[2]), obj->value[3] != 0 ? "Yes" : "No");
      send_to_char(buf, ch);
      break;
...
bool oedit_material(CHAR_DATA * ch, char *argument)
{
  ...
  {
    send_to_char("Syntax:  material [number]\n\r", ch);
    return FALSE;
  }

  pObj->material = atoi(argument);
...
bool medit_show(CHAR_DATA * ch, char *argument)
{
  ...
  sprintf(buf, "v0: Toughness         [%4d]\n\r", pMob->toughness);
  stc(buf, ch);
  sprintf(buf, "v1: Damage Modifier   [%4d]\n\r", pMob->dam_modifier);
  stc(buf, ch);
  sprintf(buf, "v2: Extra Attacks     [%4d]\n\r", pMob->extra_attack);
  stc(buf, ch);

  switch (pMob->natural_attack)
  {
    default:
      sprintf(buf, "Attack     : [Hit]\n\r");
      break;
    case 1:
      sprintf(buf, "Attack     : [Slice]\n\r");
      break;
    case 2:
      sprintf(buf, "Attack     : [Stab]\n\r");
      break;
    case 3:
      sprintf(buf, "Attack     : [Slash]\n\r");
      break;
    case 4:
      sprintf(buf, "Attack     : [Whip]\n\r");
      break;
    case 5:
      sprintf(buf, "Attack     : [Claw]\n\r");
      break;
    case 6:
      sprintf(buf, "Attack     : [Blast]\n\r");
      break;
    case 7:
      sprintf(buf, "Attack     : [Pound]\n\r");
      break;
    case 8:
      sprintf(buf, "Attack     : [Crush]\n\r");
      break;
    case 9:
      sprintf(buf, "Attack     : [Grep]\n\r");
      break;
    case 10:
      sprintf(buf, "Attack     : [Bite]\n\r");
      break;
    case 11:
      sprintf(buf, "Attack     : [Pierce]\n\r");
      break;
    case 12:
      sprintf(buf, "Attack     : [Suck]\n\r");
      break;
  }
  send_to_char(buf, ch);
